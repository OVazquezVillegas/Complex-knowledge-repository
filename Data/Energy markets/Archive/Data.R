...
Country <- unique(NUTS_country$Country)
Country <- as.data.frame(Country)

colnames(HYDRO_1)[1] <- "Country"

HYDRO_1_plot <- inner_join(Country, HYDRO_1, by = "Country")
...
# Plot
GEO_1$GEO_1_avg <- round(GEO_1$GEO_1_avg*100, digits = 1)
...
PLOT_GEO_1 <- ggplot(GEO_1, aes(reorder(GEO_1$`GEO$Country`,-GEO_1$GEO_1_avg, sum), GEO_1$GEO_1_avg)) +
  ...
  theme_few() + labs(title= "Share of electricity from GEOpower (average: 2000-2009)", 
                     y="Share", x = "Country")


print(PLOT_GEO_1)

GEO_2$GEO_2_avg <- round(GEO_2$GEO_2_avg*100, digits = 1)
...
PLOT_GEO_2 <- ggplot(GEO_2, aes(reorder(GEO_2$`GEO$Country`,-GEO_2$GEO_2_avg, sum), GEO_2$GEO_2_avg)) +
  ...
  geom_text(aes(label=GEO_2_avg), vjust=-0.5, color="black", size=3)+
   theme_few() + labs(title= "Share of electricity from goethermal sources (average: 2010-2019)", 
                     y="Share", x = "Country")
...
# Plot
PV_1$PV_1_avg <- round(PV_1$PV_1_avg*100, digits = 1)
...
# Plot
THERMAL_1$THERMAL_1_avg <- round(THERMAL_1$THERMAL_1_avg*100, digits = 1)
...
THERMAL_2$THERMAL_2_avg <- round(THERMAL_2$THERMAL_2_avg*100, digits = 1)
...
# Plot
OCEAN_1$OCEAN_1_avg <- round(OCEAN_1$OCEAN_1_avg*100, digits = 1)
...
OCEAN_2$OCEAN_2_avg <- round(OCEAN_2$OCEAN_2_avg*100, digits = 1)
