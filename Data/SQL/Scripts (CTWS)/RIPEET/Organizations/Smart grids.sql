...
SELECT a.appln_id, b.cpc_class_symbol, d.cited_npl_publn_id, e.ut
INTO #tech_all
...
where b.cpc_class_symbol LIKE  'Y04S  %'
...
SELECT cluster_id1, weight, perc_cluster = (weight /183.00) * 100.0
INTO #cluster_perc
...
SELECT DISTINCT a.cluster_id1, a.weight, perc_cluster = (a.weight /183.00) * 100.0
...
/* 5.1 Select publication years and geographical scope (European regions, 2010-2020) */ 
...
WHERE a.pub_year between 2010 and 2020 and left(e.[nuts_level3_code],4) != 'NULL'
...
SELECT nuts2=left(h.[nuts_level3_code],4), a.ut, a.cluster_id1, i.country_iso_alpha3_code
...
/* 6.2 Calculate the number of unique regional units (NUTS2) of the sample*/ 
SELECT nuts2, count(*) AS counts, country_iso_alpha3_code
FROM  #tech_ut_all_nuts
GROUP BY nuts2, country_iso_alpha3_code
ORDER BY counts DESC

/* 6.3 Calculate the number of specific regional units (NUTS2) of the sample*/ 
SELECT nuts2, count(*) AS counts
FROM #tech_ut_all_nuts
WHERE nuts2 = 'ES43' OR nuts2 = 'UKM6' OR nuts2 = 'FI19'
GROUP BY nuts2

/* 6.3 Select RIPEET sample (nuts and organization name)*/ 
DROP TABLE IF EXISTS #ripeet
SELECT DISTINCT e.nuts_level3_code, c.organization_enhanced
INTO #ripeet
FROM #tech_ut_all_nuts AS a
JOIN wos_2113..pub_affiliation_organization_enhanced AS b on a.ut = b.ut
JOIN wos_2113..organization_enhanced AS c ON c.organization_enhanced_id = b.organization_enhanced_id
JOIN wos_2113_geolocation..pub_affiliation_city_geolocation AS d ON a.ut=d.ut and b.affiliation_seq = d.affiliation_seq
JOIN wos_2113_geolocation..city_geolocation AS e ON e.city_geolocation_id = d.city_geolocation_id
WHERE e.nuts_level3_code LIKE 'ES43%' OR e.nuts_level3_code LIKE 'UKM6%' OR e.nuts_level3_code LIKE 'FI19%'
ORDER BY e.nuts_level3_code DESC

DROP TABLE IF EXISTS #organizations
SELECT organization_enhanced, freq = count(*)
INTO #organizations
FROM #ripeet
WHERE nuts_level3_code LIKE  'FI19%'
...
SELECT organization_enhanced
FROM #organizations



/* 8 Calculate the number of unique regional units (NUTS2) of the sample*/ 
DROP TABLE IF EXISTS #nuts_counts
SELECT nuts2, count(*) AS counts, country_iso_alpha3_code
INTO #nuts_counts
FROM  #tech_ut_all_nuts
GROUP BY nuts2, country_iso_alpha3_code
...
SELECT AVG(counts)
FROM #nuts_counts
WHERE country_iso_alpha3_code = 'FIN'

/* Mean UK: 91*/
SELECT AVG(counts)
FROM #nuts_counts
WHERE country_iso_alpha3_code = 'GBR'

/* Mean Spain: 109*/
SELECT AVG(counts)
FROM #nuts_counts
WHERE country_iso_alpha3_code = 'ESP'
